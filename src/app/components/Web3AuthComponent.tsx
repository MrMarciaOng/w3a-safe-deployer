...
const clientId = 'BJ-uOTw1stDYuZ7PighbkKr9UyR-_nzDmkrM87BbI9GCFb_JG215OCMXmy9-V2hTCTlICGmzP09zHFug0FxwMH4';
...
export default function Web3AuthComponent() {
  ...
  const verifySignedMessage = async (originalMessage: string, signedMessage: string) => {
    if (!provider) {
      uiConsole('provider not initialized yet');
      return;
    }
    const web3 = new Web3(provider as any);
    const recoveredAddress = await web3.eth.personal.ecRecover(originalMessage, signedMessage);
  ...
  const signMessage = async () => {
    ...
    const txObject = {
      from: fromAddress,
      to: '0x4ce32cCb81F83C447d9AD1543cE72976b4Ef243D',
      value: web3.utils.toWei('0.01', 'ether'),
      gas: 21000,
    };
    const txHash = web3.utils.soliditySha3(
      { type: 'address', value: txObject.from },
      { type: 'address', value: txObject.to },
      { type: 'uint256', value: txObject.value },
      { type: 'uint256', value: txObject.gas }
    );
    try {
      const signedMessage = await web3.eth.personal.sign(txHash as string, fromAddress, '');
      uiConsole(`Signed Message: ${signedMessage}`);
      await verifySignedMessage(txHash as string, signedMessage);
    } catch (error: any) {
